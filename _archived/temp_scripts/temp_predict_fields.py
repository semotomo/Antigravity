...
def main():
    ...
    for idx, edit_url in enumerate(entries):
        ...
        for tag in soup_edit.find_all(['input', 'textarea']):
            ...
            # text01, text02 ... textarea01 ...
            if name.startswith('text') or name.startswith('textarea') or name.startswith('date') or name.startswith('checkbox') or 'price' in name.lower() or 'sex' in name.lower():
                t = tag.name
                t_attr = tag.get('type', '')
                ...
                # 値が空でないものだけ表示
                if val or checked:
                    desc = f"Field: {name:<15} | Value: {val}"
