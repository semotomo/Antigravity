...
export async function getStoreContext(): Promise<StoreContext> {
  ...
  const rawStoreViewCookie = cookieStore.get('current_store_view')?.value
  const storeViewCookie: StoreView | undefined =
    rawStoreViewCookie === 'all' || rawStoreViewCookie === 'main' || rawStoreViewCookie === 'wanwan'
      ? rawStoreViewCookie
      : undefined

  // わんわん用アカウントはCookieを書き換えても本店の商品を参照できないよう固定する
  const currentView: StoreView =
    storeType === 'wanwan' ? 'wanwan' : storeViewCookie || 'main'
