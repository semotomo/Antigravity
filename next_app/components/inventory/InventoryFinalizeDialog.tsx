...
import type { InventoryFinalizationReadiness } from '@/lib/inventory/workspace'

type InventoryFinalizeDialogProps = {
  error: string
  pending: boolean
  posReview: InventoryFinalizationReview | null
  readiness: InventoryFinalizationReadiness | null
  onClose: () => void
    ...
    >
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-pretty text-sm text-gray-600">
              まず保存状態を高速確認し、実際の確定時にだけ最新のPOS履歴を取得します。
            </p>
          </div>
        </div>
      ...
      <div className="space-y-5 px-5 py-5 sm:px-6">
        {!readiness ? (
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
            <p className="text-pretty text-sm text-gray-700">
              確定前チェックではPOSへ接続しません。未棚卸し・除外・保存状態だけを確認し、下書き数量や現在庫は変更しません。
            </p>
          </div>
        ) : (
          <>
            <div className="grid gap-3 sm:grid-cols-4">
              {[
                ['対象商品', readiness.totalCount],
                ['棚卸し済み', readiness.countedCount],
                ['未棚卸し', readiness.pendingCount],
                ['理由付き除外', readiness.excludedCount],
              ].map(([label, value]) => (
            ...
            {readiness.canFinalize && !posReview ? (
              <div className="flex items-start gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-900">
                ...
                <p className="text-pretty">
                  入力状態に問題はありません。確定時に最新の販売・返品履歴を取得し、各商品の入力時刻以降を反映して現在庫を更新します。通常は40秒前後かかります。
                </p>
              </div>
              ...
              <div className="flex items-start gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
                ...
                <p className="text-pretty">
                  未入力の商品を計数するか、理由付きで棚卸し対象から除外してください。1商品以上の入力が必要です。
                </p>
              </div>
            ) : null}
            ...
            {posReview ? (
              <div className="space-y-3 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <div className="flex items-start gap-2 text-sm text-amber-950">
                  <AlertTriangle className="mt-0.5 size-5 shrink-0" aria-hidden="true" />
                  <p className="text-pretty">
                    入力数量は保存されていますが、最新POS履歴に確認が必要なため確定していません。未照合は商品マスタを修正し、同分時刻の商品は数量を置き換えて計数時刻を更新してください。
                  </p>
                </div>
                <div className="grid gap-2 sm:grid-cols-3">
                  {[
                    ['POS未照合', posReview.unmatchedCount],
                    ['同分時刻', posReview.ambiguousCount],
                    ['計算商品', posReview.balanceCount],
                  ].map(([label, value]) => (
                    <div key={label} className="rounded-xl border border-amber-200 bg-white p-3">
                      <p className="text-xs font-medium text-gray-600">{label}</p>
                      <p className="mt-1 text-xl font-bold tabular-nums text-gray-950">{value}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {posReview && posReview.issues.length > 0 ? (
              <div className="space-y-2">
                <h3 className="text-sm font-bold text-gray-900">確定できなかったPOS履歴</h3>
                <div className="max-h-56 space-y-2 overflow-y-auto rounded-2xl border border-gray-200 p-2">
                  {posReview.issues.map((issue) => (
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {!readiness || !readiness.canFinalize || posReview ? (
            ...
            <button type="button" onClick={() => void onFinalize()} disabled={pending} className="inline-flex items-center justify-center gap-2 rounded-xl bg-red-700 px-4 py-3 text-sm font-bold text-white hover:bg-red-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600 focus-visible:ring-offset-2 disabled:opacity-50">
              {pending ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : null}
              {pending ? '販売・返品履歴を取得中…' : '最新履歴を取得して確定する'}
            </button>
          )}
