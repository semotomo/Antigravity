...
- [ ] Next.js版「棚卸し・在庫管理」 `cc:WIP` `[feature:security]` `[feature:tdd]` `[feature:a11y]`
  ...
  - [x] Phase 5.2: 確定前チェック・確定後同期の堅牢化 `cc:完了` (2026-08-26)
    - [x] 本番のstore 6下書きで確定前チェック失敗を数量・現在庫の変更なしで再現
    - [x] 棚卸し同期だけが誤った`GAS_WEB_APP_URL`を参照し、既存設定`GAS_WEBAPP_URL`を読めない原因を特定
    - [x] POS取得・snapshot保存・確定前集計を段階別の安全なエラーへ分け、保存済み数量が維持されることを明示
    - [x] 数量保存中は確定前チェックを開始させず、保存と確定前集計の競合を防止
    - [x] 確定transaction完了後に別Actionで最新履歴を全量再計算し、同期失敗を確定失敗と混同しない
    - [x] GAS当日30件・棚卸し期間115件を各約39秒で取得し、Server Action上限300秒、回帰93/93、型、対象Lint、本番buildを確認
    - [x] commit `45fe0da`をGitHub mainへ反映し、Vercel Production Ready、本番store 6の確定前チェック成功と入力済み2件の保持を確認
  - [ ] Phase 5.3: 確定前チェックの高速化 `cc:WIP` (2026-08-26)
    - [x] 確定前チェックをSupabase内の未棚卸し・除外・保存状態確認だけにし、GAS/POS履歴取得を行わない
    - [x] 実確定時だけ最新POS履歴を1回取得し、商品別`counted_at`以降を同じsnapshotで再計算して確定する
    - [x] 同一画面の入力を停止し、他端末更新はsession row versionで確定拒否して入力数量を保持する
    - [x] POS取得・照合・確定に失敗しても理由を表示し、安全に再試行できるようにする
    - [x] 重複GAS取得を除去し、回帰97/97、型、対象Lint、本番buildで既存の二重減算防止と店舗分離を確認
    - [ ] Git/Vercel反映後、認証済み本番で高速チェックと入力数量保持を確認
