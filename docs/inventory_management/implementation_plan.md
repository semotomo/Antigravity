...
> 作成日: 2026-08-23
> 状態: **Phase 5.1本番反映・実データ確認完了 / 実機確認待ち**
...
- Phase 3追加要件として、商品行へ理由必須の停止/解除を追加する。停止状態はmanagerだけが変更でき、`store_id + JAN`をDBで再検証する。
- 手動停止はPOS商品同期より優先するoverrideとして保持し、停止済み商品も現在の棚卸しsnapshotから削除せず数量入力を継続できるようにする。
- 確定前チェックは`inventory_balances`を上書きしないpreview RPCで実行し、確定transaction内だけで正本から残高投影を再生成する。
...
- Phase 3追加要件として印刷ボタンと認証済み全件印刷画面を先行し、Phase 4の確定結果と接続して完成させる。
...
### Phase 5.2: 確定前チェック・確定後同期の堅牢化

2026-08-26、本番store 6の下書きで、確定前チェックが数量・現在庫を書き換える前に失敗する事象を再現した。原因は棚卸し同期だけが未設定名`GAS_WEB_APP_URL`を参照し、既存GAS連携とVercel設定で使用中の`GAS_WEBAPP_URL`を読めないことだった。

- 環境変数名を既存GAS連携へ統一し、移行期間は誤設定名も後方互換として受け付ける。
- POS取得、snapshot保存、確定前DB集計を段階別に扱い、入力済み数量が保持されていることをエラー文で明示する。
- 数量保存中は確定前チェックを無効化し、保存RPCとpreview RPCの競合を防ぐ。
- 確定RPCは確認済みsnapshotでtransactionを完了した後、別処理として最新POS履歴を再取得し、商品別`counted_at`から全量再集計してbalanceを上書きする。
- 確定後同期に失敗しても確定成功を取り消さず、現在庫画面の「最新履歴で再計算」から安全に再試行できる警告を返す。同じ履歴の再同期は差分加減算ではなく上書き投影なので二重減算しない。

本番DB migration、GAS変更は不要。Git/Vercel反映は差分・検証結果を提示して承認を得てから実施する。
