...
- [page.tsx (app/(dashboard)/sales/abc/page.tsx)](file:///C:/Users/kirik/Desktop/Antigravity/next_app/app/(dashboard)/sales/abc/page.tsx) / [AbcFilterControls.tsx [NEW]](file:///C:/Users/kirik/Desktop/Antigravity/next_app/components/sales/AbcFilterControls.tsx)
  - フィルターフォームをクライアントコンポーネント `AbcFilterControls` へ切り出し。
  - 「数量ベースで分析（標準）」と「売上金額ベースで分析」を切り替える直感的なトグルタブを追加。選択時にURLパラメータ（`target`）を書き換えて瞬時に再計算結果をロードします。
